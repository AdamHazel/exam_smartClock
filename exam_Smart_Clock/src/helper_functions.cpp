 ...
 * @param Reference to LCD object DFRobot_RGBLCD1602
 */
...
/**
 * @brief Clears screen when screen is changed
 * @param References to following objects: DFRobot_RGBLCD1602, bool
 * screenChange, int screenNumber
 * @see main.cpp
 */
...
/**
 * @brief Gets requested resource using HTTP protocol
 * @param size of buffer to store request, host address, requested resource
 * @see certificates_hosts.h
 * @return JSON string
 */
...
/**
 * @brief Gets requested resource using HTTPS protocol
 * @param size of buffer to store request, host address, HTTPS certificate,
 * requested resource
 * @see certificates_hosts.h
 * @return JSON string
 */
char *getInformation_https(int BUFFER_SIZE_REQUEST, const char *hostChoice,
  ...
  while (true) {
    ...
    // Loop to send HTTPS request
    while (bytes_to_send) {
      sent_bytes = socket->send(https_request + sent_bytes, bytes_to_send);
...
std::string *getInformation_http_NOTJSON(int BUFFER_SIZE_REQUEST,
  ...
  while (true) {
    ...
    printf("Step 7: Create and send HTTP request\n");
    char http_request[BUFFER_SIZE_REQUEST];
    snprintf(http_request, BUFFER_SIZE_REQUEST,
             "GET %s HTTP/1.1\r\n"
             "Host: %s\r\n"
    ...
    static char http_response[BUFFER_SIZE_RESPONSE + 1];
    memset(http_response, 0, sizeof(http_response));
    ...
    http_response[BUFFER_SIZE_RESPONSE] = '\0';

    // Prepare string to display
    char *start;
    char titleNeedle[] = "<title><![CDATA[";

    start = strstr(http_response, titleNeedle);
    start += strlen(titleNeedle);

    std::string *dynamicAlloc = new std::string;
    *(dynamicAlloc) = start;
