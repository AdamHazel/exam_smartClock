...
char* getInformation_Network(int BUFFER_SIZE_REQUEST, const char* hostChoice, const char* certificate, const char* resourceWanted)
{
    ...
    while (true)
    {
        ...
        static char https_response[BUFFER_SIZE_RESPONSE];
        memset(https_response, 0, sizeof(https_response));
        ...
        {
            nsapi_size_or_error_t result = socket->recv(https_response + received_bytes, remaining_bytes);
        ...
        char *json_end = strrchr(https_response, '}');
